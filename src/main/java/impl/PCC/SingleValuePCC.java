	...
	public Set<Rule> rules() { 	
		...
		if (separators.size() == 0) {
			...
			for (String separator : separators) {
				...
				rules.addAll(StatementNonExistenceHelper.initRequireQualifier(requiredPropertyConstant, statementEDB_SIpC, qualifierEDB_SPV, unequal_Pr));
			}
			...
			//	last_qualifier(S, P, V),
			//	require_qualifier(S, P, V, R)
			Rule doesNotHave = Expressions.makeRule(does_not_have_SR, statementEDB_SIpC, last_qualifier_SPV, require_qualifier_SPVR);
