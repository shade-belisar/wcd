...
For statements the pattern consists of two rules, both utilising the same conjunction \(c = \text{tripleEDB}(S, I, P, V) \wedge C_1 \wedge .. \wedge C_m\) with arbitrary atoms \(C_n\) which is used to test if the triple does not fulfil the requirement. Then rule one
\begin{equation*}
\text{require}(S, t) \leftarrow \text{first}(S) \wedge c
\end{equation*}
...
For qualifiers the conjunction has to be adapted to \(c_q = \text{qualifierEDB}(S, P, V) \wedge C_1 \wedge .. \wedge C_m \) and the rules are
\begin{equation*}
