...
\begin{align}
\begin{split}
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V)\\
\end{split}\\
\begin{split}
\text{violation\_qualifier}&(S, \text{propertyConstant}, V) \leftarrow \\
&\text{qualifier\_EDB}(S, \text{propertyConstant}, V)\\
\end{split}\\
\begin{split}
\text{violation\_reference}&(S, \text{propertyConstant}, V) \leftarrow \\
&\text{reference\_EDB}(S, H, \text{propertyConstant}, V)\\
\end{split}
\end{align}
...
\begin{split}
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow\\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge{}\\
&\text{statement\_EDB}(O, E, \text{conflictingPropertyConstant}, C)
\end{split}\\
\begin{split}
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow\\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge{}\\
&\text{statement\_EDB}(O, E, \text{conflictingPropertyConstant}, \text{conflictingValueConstant})
\end{split}
\end{align}
...
\begin{split}
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge \text{item}(I)
\end{split}\\
\begin{split}
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge \text{property}(I)
\end{split}
\end{align}
...
\begin{align}
\begin{split}
\text{violation\_statement}&(S, E, \text{propertyConstant}, \text{forbiddenValueConstant}) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, \text{forbiddenValueConstant})
\end{split}\\
\begin{split}
...
To save space and avoid redundancy, the following rules will not explicitly state the inequality requirement predicate. Instead, assume that for every premise of the form \(B_1 \wedge \ldots\wedge B_m \wedge I_1 \wedge I_n\) where \(B_1 .. B_m\) are arbitrary atoms, \(I_1 .. I_n\) are atoms of the form \(\text{unequal}(t_{n_1}, t_{n_2})\) with \(t_{n_1}, t_{n_2}\) terms and all variables amount \(t_{n_1}\) and \(t_{n_2}\) appear in at least one of \(B_1 \ldots B_m\), rule one is introduced for all \(n\). The rules outlined in Section \ref{subsubsec_encoding} are changed to utilise this new requirement as shown in rule two.
\begin{align}
\begin{split}
...
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge{} \\
&\text{unit}(V, U)  {\textstyle\bigwedge}_{a \in A} \text{unequal}(a, U)
\end{split}\\
\begin{split}
\text{violation\_qualifier}&(S, \text{propertyConstant}, V) \leftarrow \\
...
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge{} \\
&\text{qualifier\_EDB}(S, Q, O) {\textstyle\bigwedge}_{a \in A} \text{unequal}(a, Q)
\end{split}
\end{align}
...
\begin{split}
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) {\textstyle\bigwedge}_{a \in A} \text{unequal}(a, V)
\end{split}\\
\begin{split}
\text{violation\_qualifier}&(S, \text{propertyConstant}, V) \leftarrow \\
&\text{qualifier\_EDB}(S, \text{propertyConstant}, V) {\textstyle\bigwedge}_{a \in A} \text{unequal}(a, V)
\end{split}\\
\begin{split}
\text{violation\_reference}&(S, \text{propertyConstant}, V) \leftarrow \\
&\text{reference\_EDB}(S, H, \text{propertyConstant}, V) {\textstyle\bigwedge}_{a \in A} \text{unequal}(a, V)
\end{split}
\end{align}
...
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge{} \\
&\text{qualifier\_EDB}(S, \text{qualifierProperty}, O) {\textstyle\bigwedge}_{a \in A} \text{unequal}(a, O)
\end{split}
\end{align}
...
For statements the pattern consists of two rules, both utilising the same conjunction \(\text{doesNotFulfil}\) whose atoms depend on the constraint type. This conjunction is used to test if the statement does not fulfil the requirement. Then rule one states that the statement requires the condition and rule two propagates this down the statement chain. For qualifiers the rules three and four apply.
\begin{align}
&\text{require}(S, \text{id}) \leftarrow \text{first}(S, I) \wedge \text{doesNotFulfil} \\
&\text{require}(S, \text{id}) \leftarrow \text{next}(O, S) \wedge \text{require}(O, \text{id}) \wedge \text{doesNotFulfil} \\
&\text{require}(S, P, V, \text{id}) \leftarrow \text{first}(S, P, V) \wedge \text{doesNotFulfil} \\
&\text{require}(S, P, V, \text{id}) \leftarrow \text{next}(O, X, C, S, P, V) \wedge \text{require}(O, X, C, \text{id}) \wedge \text{doesNotFulfil}
\end{align}
...
\paragraph{Rules}
The \emph{item requires statement} constraint requires inequality between all properties of statements and the \emph{required property} as well as between all statement values and the \emph{allowed values}. The order of the item statements needs to be established. An item violates the constraint if it a) has a statement using the constrained property as predicate, b1) has no statements using the \emph{required property} as predicate, or (if \emph{allowed values} are specified) b2) has only statement using the \emph{required property} as predicate that do not use one of the \emph{allowed values} as value. If no \emph{allowed values} have been specified for the declared constraint the conjunction for the require pattern is one. If a set of \emph{allowed values} has been specified, a second set of require-rules have to be introduced with conjunction two. Note that the require pattern using conjunction one is still required, since the preceding statements do not need to have the \emph{required property}. In both cases violations can be found with rule three

\begin{align}
\begin{split}
c = &\text{statement\_EDB}(Q, E, \text{propertyConstant}, X) \wedge{} \\
&\text{statement\_EDB}(S, E, P, V) \wedge \text{unequal}(\text{requiredPropertyConstant}, P)
\end{split} \\
\begin{split}
c = &\text{statement\_EDB}(Q, E, \text{propertyConstant}, X) \wedge{} \\
&\text{statement\_EDB}(S, E, \text{requiredPropertyConstant}, V) \wedge {\textstyle\bigwedge}_{a \in A} \text{unequal}(a, V)
\end{split}
\end{align}
\begin{align}
\begin{split}
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge{} \text{last}(O, I) \wedge{} \\
...
\begin{align}
\begin{split}
c = &\text{statement\_EDB}(Q, R, \text{propertyConstant}, I) \wedge{} \\
&\text{statement\_EDB}(S, E, P, V) \wedge \text{unequal}(\text{requiredPropertyConstant}, P)
\end{split}\\
\begin{split}
c = &\text{statement\_EDB}(Q, R, \text{propertyConstant}, I) \wedge{} \\
...
\begin{split}
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge{} \text{last}(O, V) \wedge{} \\
&\text{require}(O, \text{propertyConstant}, \text{requiredPropertyConstant})
\end{split}
\end{align}
...
\begin{split}
c = &\text{statement\_EDB}(Q, R, \text{propertyConstant}, I) \wedge{} \\
&\text{statement\_EDB}(S, E, P, V) \wedge{} \\
&\text{unequal}(\text{inversePropertyConstant}, P)
\end{split}\\
\begin{split}
c = &\text{statement\_EDB}(Q, R, \text{propertyConstant}, I) \wedge{} \\
&\text{statement\_EDB}(S, E, \text{inversePropertyConstant}, V) \wedge{} \\
&\text{unequal}(R, V)
\end{split}
\end{align}
...
\begin{split}
c = &\text{statement\_EDB}(Q, E, \text{propertyConstant}, X) \wedge{} \\
&\text{statement\_EDB}(S, E, P, V) \wedge \text{unequal}(\text{propertyConstant}, P)
\end{split}
\end{equation}
The predicate require\_second\([2]\) is introduced with the same meaning as require except that it will be applied after a required property has been found. A violation would be any last statement requiring a second constrained statement, analogous to the violation rule for \emph{item requires statement} (see Section \ref{subsec_item_requires_statement}).
\begin{align}
\begin{split}
\text{require\_second}&(S, \text{propertyConstant}, \text{propertyConstant}) \leftarrow \\
&\text{next}(R, S) \wedge \text{statement\_EDB}(R, E, \text{propertyConstant}, C) \wedge{} \\
...
\text{violation\_statement}&(S, E, \text{propertyConstant}, V) \leftarrow \\
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge{} \\
&\text{statement\_EDB}(O, E, \text{propertyConstant}, X) \wedge{} \\
&\text{unequal}(S, O)
\end{split}\\
\begin{split}
\text{violation\_qualifier}&(S, \text{propertyConstant}, V) \leftarrow \\
...
&\text{statement\_EDB}(S, E, \text{propertyConstant}, X) \wedge{} \\
&\text{statement\_EDB}(O, E, \text{propertyConstant}, Y) \wedge{} \\
...
&\text{statement\_EDB}(S, E, \text{propertyConstant}, V) \wedge{} \\
&\text{rank\_EDB}(S, \text{preferredRankConstant}) \wedge{} \\
&\text{statement\_EDB}(O, E, \text{propertyConstant}, W) \wedge{} \\
&\text{rank\_EDB}(O, \text{preferredRankConstant}) \wedge{} \\
&\text{unequal}(S, O) \wedge{} \\
