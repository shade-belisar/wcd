...
{\bf Total Time} is the execution time taken for evaluating one constraint. This does not include the data extraction from JSON, nor the generation of the files necessary to establish inequality. It does include the time taken to generate the rules as well as to load, reason and query with VLog.
...
{\bf Derivations} counts how many new facts were created during materialization. This will include all found violations but can be more in cases where additional facts were created as helpers.
...
Most constraints could be evaluated in under two hours on even the largest dataset. The exceptions are \emph{distinct values}, \emph{item requires statement} and \emph{single value}. In parts this is expected behaviour, since these three constraints are applied to an order of magnitude more properties than all other properties with the exception of \emph{scope}, which is described in very simple rules. However, as seen in Figure \ref{fig_runtime}, the runtimes of these three constraints are multiple orders of magnitude higher than those of other constraints. The following sections will thus focus on the comparison of these three constraints to identify bottlenecks and reasons for this huge difference in performance.
