...
unitEDB\([2]\) connects a value to its unit.\\
...
\subsubsection{Rules}
To handle this constraint inequality between all units need to be established. Then a violation can be identified by finding all statementEDB, qualifierEDB and referenceEDB with the constrained property and a unit unequal to all allowed units.
...
\begin{equation*}
\text{statementEDB}(S, I, \text{propertyConstant}, V) \wedge \text{unitEDB}(V, U)
\end{equation*}
