...
For statements the pattern consists of two rules, both utilising the same conjunction \(c\) whose atoms depend on the constraint type. This conjunction is used to test if the statement does not fulfil the requirement. Then rule one states that the statement requires the condition and rule two propagates this down the statement chain. For qualifiers the rules three and four apply.
\begin{align}
&\text{require}(S, \text{id}) \leftarrow \text{first}(S, I) \wedge c \\
&\text{require}(S, \text{id}) \leftarrow \text{next}(O, S) \wedge \text{require}(O, \text{id}) \wedge c \\
&\text{require}(S, P, V, \text{id}) \leftarrow \text{first}(S, P, V) \wedge c \\
&\text{require}(S, P, V, \text{id}) \leftarrow \text{next}(O, X, C, S, P, V) \wedge \text{require}(O, X, C, \text{id}) \wedge c
\end{align}
...
\begin{align}
\begin{split}
c_1 = &\text{statement\_EDB}(Q, E, \text{propertyConstant}, X) \wedge{} \\
&\text{statement\_EDB}(S, E, P, V) \wedge \text{unequal}(\text{requiredPropertyConstant}, P)
\end{split} \\
\begin{split}
c_2 = &\text{statement\_EDB}(Q, E, \text{propertyConstant}, X) \wedge{} \\
