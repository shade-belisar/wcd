...
\subsection{Property scope (Q53869507)}
The property scope constraint requires no context information to be violated; the triple, qualifier, or reference with the constrained property is enough. The dependent atoms are thus:
\[\text{tripleEDB}(S, I, \text{propertyConstant}, V)\]
\[\text{qualifierEDB}(S, \text{propertyConstant}, V)\]
\[\text{referenceEDB}(S, \text{propertyConstant}, V)\]
depending on if the property scope qualifier is not set to as main value, as qualifier, or as reference respectively.\\
...
\subsection{Conflicts-with (Q21502838)}
The conflicts-with constraint is violated if another statement on the same item either has a conflicting property or a conflicting property and a conflicting value depending on specified qualifiers. It depends on a conjunction (\(O\) denoting another statement ID, \(C\) another value)
\begin{equation*}
\text{tripleEDB}(S, I, \text{propertyConstant}, V) \wedge \text{tripleEDB}(O, I, \text{conflictingPropertyConstant}, C)
\end{equation*}
\begin{equation*}
\begin{split}
...
A violation for a conflicts-with constraint is thus either
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
for constraints with no conflicting values specified and one
\begin{equation*}
\begin{split}
\text{violation\_triple}&(S, I, \text{propertyConstant}, V) \leftarrow\\
&\text{tripleEDB}(S, I, \text{propertyConstant}, V) \wedge\\
...
All rules except the four covered in the previous section require inequality in some way or another. 
