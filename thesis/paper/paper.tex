...
statementEDB\([4]\) represents a single statement consisting of the statement ID, the item ID, the property ID and the value as either an item ID or a string-representation of the data value.\\
...
Since only three possibilities exist, this is equivalent to forbidding all other possibilities and the rules are simple mapping all of statementEDB, qualifierEDB and referenceEDB with the constrained property that are not allowed.
...
\end{tabularx}
\end{table}
The rule modelling can simply be done by choosing all statementEDB with the constrained property and then all statementEDB on the same item with the conflicting property, specifying the values as well if necessary.
...
Since there are only three options (and only two in use), violations can be found without inequality by finding the statementEDB with the constrained property whose subject is of the type that is not allowed, meaning either wikibase item or wikibase property.
